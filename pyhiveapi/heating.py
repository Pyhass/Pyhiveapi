...
class Heating:
    ...
    async def current_temperature(self, device):
        ...
        await self.log.log(device["hive_id"], self.type + "_Extra", "Getting current temp")
        f_state = None
        state = None
        final = None
    ...
    async def minmax_temperatures(self, device):
        """Min/Max Temp"""
        await self.log.log(device["hive_id"], self.type + "_Extra", "Getting Min/Max temp")
        state = None
        final = None

        if device["hive_id"] in Data.p_minmax:
            state = Data.p_minmax[device["hive_id"]]
            await self.log.log(device["hive_id"], self.type + "_Extra", "Min/Max is {0}", info=[state])
            final = state
        else:
            await self.log.error_check(device["hive_id"], "ERROR", "Failed")
    ...
    async def target_temperature(self, device):
        ...
        await self.log.log(device["hive_id"], self.type + "_Extra", "Getting target temp")
        state = None
        final = None
    ...
    async def get_mode(self, device):
        ...
        if device["hive_id"] in Data.products:
            ...
            await self.log.log(device["hive_id"], self.type + "_Extra", "Mode is {0}", info=[str(state)])
    ...
    async def get_state(self, device):
        ...
        if device["hive_id"] in Data.products:
            ...
            if current_temp < target_temp:
                ...
                state = "OFF"
            await self.log.log(device["hive_id"], self.type + "_Extra", "State is {0}", info=[str(state)])
            final = Data.HIVETOHA[self.type].get(state, state)
        else:
            await self.log.error_check(device["hive_id"], "ERROR", "Failed")
    ...
    async def current_operation(self, device):
        ...
        await self.log.log(device["hive_id"], self.type + "_Extra", "Getting current operation")
        state = None
        final = None
    ...
    async def boost(self, device):
        ...
        await self.log.log(device["hive_id"], self.type + "_Extra", "Getting boost status")
        state = None
        final = None
    ...
    async def get_schedule_now_next_later(self, device):
        ...
        await self.log.log(device["hive_id"], self.type + "_Extra", "Getting schedule")
        ...
        if device["hive_id"] in Data.products:
            ...
            await self.log.error_check(device["hive_id"], self.type + "_Extra", state)
            final = state
        else:
            await self.log.error_check(device["hive_id"], "ERROR", "Failed")
    ...
    async def turn_boost_on(self, device, mins, temp):
        ...
        if mins > 0 and temp >= await self.min_temperature(device):
            if temp <= await self.max_temperature(device):
                await self.log.log(device["hive_id"], self.type + "_Extra", "Enabling boost for {0}")
                await self.session.hive_refresh_tokens()
                final = False
