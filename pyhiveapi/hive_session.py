...
class Session:
    ...
    async def open_file(self, file):
        ...
        async with AIOFile(path, 'a') as afp:
            while True:
                json_data = await afp.read(4096)
        data = await json.loads(json_data)
