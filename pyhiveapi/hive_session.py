...
class Session:
    ...
    async def add_list(self, name, data, **kwargs):
        ...
        if add:
            formatted_data = {}
            try:
                formatted_data = {"hiveID": data.get("id", ""),
                ...
                if kwargs.get("haName", "FALSE")[0] == " ":
                    kwargs["haName"] = data.get("state", {}).get(
                        "name", "") + kwargs["haName"]
                else:
                    formatted_data["haName"] = formatted_data["hiveName"]
                formatted_data.update(kwargs)
            except (KeyError):
                await self.logger.log("No_ID", self.sessionType, "Could not setup device - " + str(data))
    ...
    async def hiveRefreshTokens(self):
        ...
        if Data.file:
            await self.logger.log("No_ID", self.sessionType, "useFile is active - Cannot refresh tokens.")
            return None
        else:
            await self.logger.log("No_ID", self.sessionType, "Checking if new tokens are required")

            expiry_time = Data.tokenCreated + Data.tokenExpiry
            if datetime.now() >= expiry_time:
                await self.logger.log("No_ID", self.sessionType, "Attempting to refresh tokens.")
    ...
    async def startSession(self, config):
        ...
        if config['tokens'] is not None and not Data.file:
            ...
            Data.tokens.update(
                {"token": config['tokens']["IdToken"]})
            Data.tokens.update(
                {"refreshToken": config['tokens']["RefreshToken"]})
            Data.tokens.update(
                {"accessToken": config['tokens']["AccessToken"]})
        elif Data.file:
            await self.logger.log("No_ID", self.sessionType, "Loading up a hive session with a preloaded file.")
        else:
        ...
        for aProduct in Data.products:
            ...
            if Data.products[aProduct]["type"] == "sense":
                await self.add_list("binary_sensor", p, haName="Glass Detection",
                                    hiveType="GLASS_BREAK",
                                    device_id=p["parent"], device_name=p["state"]["name"])
                await self.add_list("binary_sensor", p, haName="Smoke Detection",
                                    hiveType="SMOKE_CO",
                                    device_id=p["parent"], device_name=p["state"]["name"])
                await self.add_list("binary_sensor", p, haName="Dog Bark Detection",
                                    hiveType="DOG_BARK",
                                    device_id=p["parent"], device_name=p["state"]["name"])
            ...
            if Data.products[aProduct]["type"] in Data.HIVE_TYPES["Heating"]:
                ...
                await self.add_list("sensor", p, haName=" Current Temperature",
                                    hiveType="CurrentTemperature", device_id=device_id,
                                    device_name=device_name, custom=True)
                await self.add_list("sensor", p, haName=" Target Temperature",
                                    hiveType="TargetTemperature", device_id=device_id,
                                    device_name=device_name, custom=True)
                await self.add_list("sensor", p, haName=" State",
                                    hiveType="Heating_State", device_id=device_id,
                                    device_name=device_name, custom=True)
                await self.add_list("sensor", p, haName=" Mode",
                                    hiveType="Heating_Mode", device_id=device_id,
                                    device_name=device_name, custom=True)
                await self.add_list("sensor", p, haName=" Boost",
                                    hiveType="Heating_Boost", device_id=device_id,
                                    device_name=device_name, custom=True)
            ...
            if Data.products[aProduct]["type"] in Data.HIVE_TYPES["Hotwater"]:
                ...
                await self.add_list("sensor", p, haName="Hotwater State",
                                    hiveType="Hotwater_State", device_id=device_id,
                                    device_name=device_name, custom=True)
                await self.add_list("sensor", p, haName="Hotwater Mode",
                                    hiveType="Hotwater_Mode", device_id=device_id,
                                    device_name=device_name, custom=True)
                await self.add_list("sensor", p, haName="Hotwater Boost",
                                    hiveType="Hotwater_Boost", device_id=device_id,
                                    device_name=device_name, custom=True)
            ...
            if Data.products[aProduct]["type"] in Data.HIVE_TYPES["Switch"]:
                ...
                await self.add_list("sensor", p, haName=" Mode", hiveType="Mode", device_id=p["id"],
                                    device_name=p["state"]["name"], custom=True)
                await self.add_list("sensor", p, haName=" Availability", hiveType="Availability", device_id=p["id"],
                                    device_name=p["state"]["name"], custom=True)
        ...
        for aDevice in Data.devices:
            ...
            if (
                ...
                await self.add_list("sensor", d, haName=" Battery Level",
                                    hiveType="Battery", device_id=d["id"], device_name=d["state"]["name"])
                await self.add_list("sensor", d, haName=" Availability",
                                    hiveType="Availability", device_id=d["id"], device_name=d["state"]["name"],
                                    custom=True)
        ...
        if "action" in Data.HIVE_TYPES["Switch"]:
            for action in Data.actions:
                ...
                await self.add_list("switch", a, hiveName=a["name"],
                                    haName=a["name"],
                                    hiveType="action")

        await self.logger.log("No_ID", self.sessionType, "Hive component has initialised")
